...
export default function JobGpsDrawer({ report, lang, onClose, onReportUpdated, onRouteAssigned }) {
  ...
  const [workPeriodGpsLoading, setWorkPeriodGpsLoading] = useState(true);
  const [workPeriodGpsError, setWorkPeriodGpsError] = useState('');
  const [workPeriodGpsRefreshKey, setWorkPeriodGpsRefreshKey] = useState(0);
  ...
  useEffect(() => {
    let active = true;
    const controller = new AbortController();
    setWorkPeriodGps(null);
    setWorkPeriodGpsLoading(true);
    setWorkPeriodGpsError('');
    adminFetchWorkPeriodGpsData(report.id, { signal: controller.signal })
      .then(data => { if (active) setWorkPeriodGps(data); })
      .catch(errorValue => {
        if (active && errorValue?.name !== 'AbortError') setWorkPeriodGpsError(t.periodMapFailed);
      })
      .finally(() => { if (active) setWorkPeriodGpsLoading(false); });
    return () => { active = false; controller.abort(); };
  }, [report.id, t.periodMapFailed, workPeriodGpsRefreshKey]);
  ...
  return <div className="gps-drawer-layer">
    ...
    <aside className="gps-drawer" role="dialog" aria-modal="true" aria-labelledby="gps-detail-title">
      ...
      <div className="gps-drawer-scroll">
        ...
        <section className="gps-route-section">
          <div className="gps-route-heading"><h2>{t.routeMap}</h2>{activeDetail?.route ? <a href={activeDetail.route.googleMapsUrl} target="_blank" rel="noreferrer">{t.openRoute}<ArrowSquareOutIcon size={14} weight="bold" aria-hidden="true" /></a> : null}</div>
          <p className={`gps-work-period-summary${workPeriodGpsLoading ? ' is-loading' : workPeriodGpsError ? ' is-error' : ''}`} role={workPeriodGpsError ? 'alert' : 'status'}>
