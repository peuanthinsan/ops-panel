...
function TimelineBar({ rows, lang, speedSeries }) {
  ...
  return <div className="print-timeline-track" role="img" aria-label={label} style={{ height: `${Math.max(16, laneCount * 7)}px` }}>
    ...
    <SpeedTimelineOverlay reports={completedRows} samplesByReportId={speedSeries.samplesByReportId} telemetryByReportId={speedSeries.telemetryByReportId} loading={speedSeries.loading} telemetryLoading={speedSeries.telemetryLoading} lang={lang} interactive={false} />
  </div>;
}
...
export function LandscapePrintDashboard({ filters = {}, lang: requestedLang, timelineOnly = false, timelineFilters = null }) {
  ...
  const timelinePages = useMemo(() => {
    const reportDay = report => stableFilters.dateBasis === 'job'
      ? reportDateKey(report.startTime)
      : report.workPeriodDate || reportDateKey(report.workPeriodStartTime || report.startTime);
    const reportDates = [...new Set(timelineReports.map(reportDay).filter(Boolean))].sort();
    ...
    for (const date of dates) {
      const dayReports = timelineReports.filter(report => reportDay(report) === date);
  ...
  if (loading || speedSeries.loading || speedSeries.telemetryLoading || error) return <PrintState lang={lang} loading={loading || speedSeries.loading || speedSeries.telemetryLoading} error={error} onRetry={load} />;
  ...
  return <main className="print-preview landscape-preview">
    ...
    {timelinePages.map((page, pageIndex) => {
      ...
      <div className="landscape-header-row"><PrintHeader compact subtitle={`${lang === 'th' ? 'สรุปรถทุกคันรายวัน' : 'Daily fleet summary'} · ${longDate(page.date, lang)}`} /><div className="print-legend">{operationActions.map(([number, thai, english]) => <span key={number}><i style={{ backgroundColor: reportModeColor(english) }} aria-hidden="true" /><b>{number}</b>{lang === 'th' ? thai : english}</span>)}<span><i className="legend-speed" />{lang === 'th' ? 'ความเร็ว (กม./ชม.)' : 'Speed (km/h)'}</span><span><i className="fuel" />{lang === 'th' ? 'น้ำมันรวม (ยังไม่ยืนยันหน่วย)' : 'Total fuel (unit unconfirmed)'}</span></div></div>
...
export function PortraitPrintDashboard({ date: requestedDate, startDate: requestedStartDate, endDate: requestedEndDate, dateBasis: requestedDateBasis, workPeriodId: requestedWorkPeriodId, vehicle: requestedVehicle, lang: requestedLang, style: requestedStyle }) {
  ...
  const usesJobDates = requestedDateBasis === 'job';
  const portraitFilters = useMemo(() => ({
    ...(workPeriodId ? { workPeriodId } : usesJobDates ? {} : { startDate: fallbackDate, endDate: fallbackDate }),
    ...(startDate ? { startDate } : {}),
    ...(endDate ? { endDate } : {}),
    ...(usesJobDates ? { dateBasis: 'job' } : {}),
    ...(vehicle ? { vehicle: [vehicle] } : {}),
  }), [endDate, fallbackDate, startDate, usesJobDates, vehicle, workPeriodId]);
  ...
  const date = (usesJobDates ? reportDateKey(summary.rows[0]?.startTime) || startDate : summary.rows[0]?.workPeriodDate) || reportDateKey(summary.start) || fallbackDate;
  ...
  const model = {
    ...
    telemetryByReportId: speedSeries.telemetryByReportId,
  ...
  return <main className={`print-preview operation-report-preview ${reportStyle === 'classic' ? 'classic-preview' : 'portrait-preview'}`} data-report-style={reportStyle}>
    ...
    <section className="print-sheet portrait-sheet">
      ...
      <DailyTripInfo lang={lang} vehicle={vehicle} summary={summary} date={date} usesJobDates={usesJobDates} />
      ...
      <section className="report-section"><div className="report-section-heading"><h2>{lang === 'th' ? 'ไทม์ไลน์การเดินรถ' : 'TRIP TIMELINE'}</h2><strong className={alerts.length ? 'timeline-alert-count' : 'timeline-clear'}><WarningIcon weight="bold" aria-hidden="true" /> {alerts.length} {lang === 'th' ? 'การแจ้งเตือนระหว่างทาง' : alerts.length === 1 ? 'alert during this trip' : 'alerts during this trip'}</strong></div><div className="timeline-legend">{operationActions.map(([number, thai, english]) => <span key={number}><i style={{ backgroundColor: reportModeColor(english) }} /><b>{number}</b>{lang === 'th' ? thai : english}</span>)}<span><i className="speed" />{lang === 'th' ? 'ความเร็ว (กม./ชม.)' : 'Speed (km/h)'}</span><span><i className="fuel" />{lang === 'th' ? 'น้ำมันรวม (ยังไม่ยืนยันหน่วย)' : 'Total fuel (unit unconfirmed)'}</span><span><i className="alert" />{lang === 'th' ? 'การแจ้งเตือน' : 'Alert'}</span></div><div className="trip-timeline" style={{ height: `${Math.max(18, 10 + laneCount * 4.5)}mm` }} role="group" aria-label={lang === 'th' ? 'ไทม์ไลน์การเดินรถพร้อมกราฟความเร็ว น้ำมันรวม และการแจ้งเตือน' : 'Trip timeline with speed, Total fuel and alerts'}>{timelineRows.map(({ report, left, width, lane }) => <span key={report.id || report.startTime} className="timeline-segment" role="img" aria-label={`${modeLabel(report, lang)} · ${time(report.startTime, lang)}–${time(report.endTime, lang)}`} title={`${modeLabel(report, lang)} · ${time(report.startTime, lang)}–${time(report.endTime, lang)}`} style={{ left: `min(${left}%, calc(100% - 2mm))`, width: `max(2mm, ${width}%)`, bottom: `${1.8 + lane * 4.5}mm`, background: reportModeColor(report.mode) }} />)}<SpeedTimelineOverlay reports={summary.completedRows} samplesByReportId={speedSeries.samplesByReportId} telemetryByReportId={speedSeries.telemetryByReportId} loading={speedSeries.loading} telemetryLoading={speedSeries.telemetryLoading} lang={lang} startMinute={0} endMinute={timelineSpanMinutes} originTime={timelineOrigin} className="print-speed-overlay" interactive={false} /><TimelineAlertMarkers alerts={alerts} lang={lang} startMinute={0} endMinute={timelineSpanMinutes} interactive={false} /></div><div className="timeline-axis">{periodAxis(timelineOrigin, Number.isFinite(timelineEnd) ? new Date(timelineEnd).toISOString() : '', lang).map((label, index) => <span key={`${label}-${index}`}>{label}</span>)}</div><TimelineAlertChips alerts={alerts} lang={lang} limit={3} className="alert-list" /></section>
      <section className="report-section job-section"><div className="report-section-heading"><h2>{lang === 'th' ? 'รายการงาน' : 'JOB LIST'}</h2><span>{dailyJobRangeLabel(lang, 1, jobPages.firstPage.length, summary.rows.length)}</span></div><DailyJobTable rows={jobPages.firstPage} lang={lang} date={date} />{!summary.rows.length ? <p className="print-empty">{lang === 'th' ? 'ไม่มีงานที่บันทึกสำหรับรถและวันที่นี้' : 'No saved jobs for this vehicle and date.'}</p> : null}</section>
    ...
    {jobPages.continuationPages.map((pageRows, pageIndex) => {
      ...
      return <section className="print-sheet portrait-sheet report-continuation-sheet" key={`daily-jobs-${pageNumber}`}>
        ...
        <DailyTripInfo lang={lang} vehicle={vehicle} summary={summary} date={date} usesJobDates={usesJobDates} />
        <section className="report-section job-section continuation-job-section"><div className="report-section-heading"><h2>{lang === 'th' ? 'รายการงาน (ต่อ)' : 'JOB LIST — CONTINUED'}</h2><span>{dailyJobRangeLabel(lang, start, end, summary.rows.length)}</span></div><DailyJobTable rows={pageRows} lang={lang} date={date} /></section>
