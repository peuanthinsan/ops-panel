...
export default function FullReportDashboard({ lang }) {
  ...
  function printReports() {
    ...
    printVehicle({ vehicleNumber: vehicles[0], workPeriodId: selectedPrintPeriodId });
  }
  function printVehicle(report) {
    ...
    const params = new URLSearchParams({ vehicle, workPeriodId, lang, dateBasis: 'job' });
  ...
  return (
    <main className="main report-workspace" id="main-content" tabIndex={-1}>
      ...
      {selectedReport ? <JobGpsDrawer key={selectedReport.id} report={selectedReport} lang={lang} onClose={closeSelectedReport} onReportUpdated={handleReportUpdated} onRouteAssigned={handleRouteAssigned} onOpenReport={setSelectedReport} onPrintWorkPeriod={printVehicle} /> : null}
    </main>
  );
}
