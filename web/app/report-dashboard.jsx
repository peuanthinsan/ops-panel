...
export default function FullReportDashboard({ lang }) {
  ...
  function printReports() {
    if (!selectedPrintPeriodId || vehicles.length !== 1) return;
    const params = new URLSearchParams({ workPeriodId: selectedPrintPeriodId, lang });
  ...
  function printVehicle(report) {
    ...
    const workPeriodId = String(report.workPeriodId || '').trim();
    if (!vehicle || !workPeriodId) return;
  ...
  const matchingPrintPeriodIds = [...new Set(reports.map(report => String(report.workPeriodId || '').trim()).filter(Boolean))];
  const selectedPrintPeriodId = matchingPrintPeriodIds.length === 1 ? matchingPrintPeriodIds[0] : '';
  const canPrintDailyReport = Boolean(selectedPrintPeriodId && vehicles.length === 1);
  const dailyPrintVehicleMessage = vehicles.length !== 1
    ? t.printVehicleRequired
    : matchingPrintPeriodIds.length > 1
    ...
    <main className="main report-workspace" id="main-content" tabIndex={-1}>
      <div className="page-header report-header">
        ...
        <div className="header-actions">
          <button className="secondary refresh-button" type="button" aria-label={loading ? t.refreshing : t.refresh} title={loading ? t.refreshing : t.refresh} aria-busy={loading} onClick={() => void loadReports()} disabled={loading}><ArrowsClockwiseIcon size={17} weight="bold" aria-hidden="true" /></button>
      ...
      <section className="panel report-panel" aria-busy={loading || search !== deferredSearch}>
        ...
        {visibleReports.length ? <div className="table-wrap" tabIndex={0} aria-label={lang === 'en' ? 'Scrollable saved jobs table' : 'ตารางงานที่บันทึก เลื่อนได้'}>
          <table className="reports-table">
            ...
            <tbody>{renderedReports.map(report => <tr key={report.id} className={`${report.status === 'Cancelled' ? 'row-cancelled' : isLookupPending(report) ? 'row-queued' : ''} ${selectedReport?.id === report.id ? 'row-selected' : ''}`}>
              ...
              <td className="status-cell"><span className={`status status-${statusSlug(report.status)}`}>{displayStatus(report.status, lang)}</span></td>
        ...
        {visibleReports.length ? <div className="report-cards" role="list" aria-label={lang === 'en' ? 'Saved jobs' : 'งานที่บันทึก'}>
          {renderedReports.map(report => <article className={`report-card ${report.status === 'Cancelled' ? 'row-cancelled' : isLookupPending(report) ? 'row-queued' : ''} ${selectedReport?.id === report.id ? 'row-selected' : ''}`} key={report.id} role="listitem" aria-label={`${t.report} ${report.id}`}>
            <div className="report-card-heading"><div><h3>{report.vehicleNumber || '—'}</h3><small>{report.id}</small>{report.workPeriodDate && report.workPeriodDate !== reportDateKey(report.startTime) ? <small>{t.workStarted}: {formatReportDate(report.workPeriodDate, lang)}</small> : null}</div><span className={`status status-${statusSlug(report.status)}`}>{displayStatus(report.status, lang)}</span></div>
