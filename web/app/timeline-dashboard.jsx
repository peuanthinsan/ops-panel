...
const printPeriodGuidanceCopy = { en: 'Choose a work period below. Printing uses the selected date range.', th: 'เลือกรอบงานด้านล่าง รายงานที่พิมพ์จะใช้ช่วงวันที่ที่เลือก' };
...
export default function TimelineDashboard({ lang, embedded = false, sourceReports = null, sourceLoading = false, sourceError = '', sharedFilters = null, refreshKey = 0, routeAssignment = null, onOpenReport = null, onPrintWorkPeriod = null, printPeriodGuidance = false }) {
  ...
  const loadReports = useCallback(async ({ silent = false, networkOnly = false } = {}) => {
    ...
    try {
      ...
      const requestFilters = embedded ? sharedQuery : { startDate: targetStartDate, endDate: targetEndDate, dateBasis: 'job' };
  ...
  const rows = useMemo(() => {
    ...
    for (let reportIndex = 0; reportIndex < timelineReports.length; reportIndex += 1) {
      ...
      const periodDate = report.workPeriodDate || reportDateKey(report.workPeriodStartTime || report.startTime);
      const actualDay = reportDateKey(report.startTime);
      const day = embedded && sharedQuery.dateBasis !== 'job' ? periodDate : actualDay;
      ...
      if (!grouped.has(key)) grouped.set(key, { groupKey: key, date: day, actualDate: actualDay, dayOffset: calendarDayOffset(periodDate, actualDay), periodStart: Date.parse(report.workPeriodStartTime || report.startTime) || 0, vehicle: report.vehicleNumber || '—', driver: report.driverName || '—', order: reportIndex, reports: [] });
  ...
  function printTimeline() {
    ...
    const params = new URLSearchParams({ view: 'timeline', lang, startDate: printStartDate, endDate: printEndDate, dateBasis: 'job' });
  ...
  return (
    <TimelineWorkspace className={embedded ? 'timeline-embedded-workspace' : 'main timeline-workspace'} id={embedded ? undefined : 'main-content'} tabIndex={embedded ? undefined : -1} aria-label={embedded ? t.title : undefined}>
      <div className={embedded ? 'section-heading timeline-embedded-heading' : 'page-header'}>
        ...
        {embedded && onOpenReport ? <div className="timeline-inspect-hint"><CrosshairSimpleIcon size={17} weight="bold" aria-hidden="true" /><span>{t.inspectHint}</span></div> : null}
      ...
      <section className="panel timeline-panel" aria-busy={timelineLoading}>
        ...
        <div className={`timeline-scroll ${printPeriodGuidance ? 'print-period-guidance-active' : ''}`} id="timeline-results" tabIndex={0} aria-label={lang === 'en' ? 'Scrollable vehicle activity timeline' : 'ไทม์ไลน์กิจกรรมรถ เลื่อนได้'}>
          ...
          {timelinePage.items.map(row => { const rowAlerts = alertsByRow.get(row) || []; return <div className={`timeline-grid timeline-row ${rowAlerts.length ? 'timeline-row-has-alerts' : ''}`} key={row.groupKey} role="group" aria-label={`${t.vehicle}: ${row.vehicle}. ${t.driver}: ${row.driver}. ${t.date}: ${formatReportDate(row.actualDate, lang)}. ${rowAlerts.length} ${t.alert}.`}><div><strong>{row.vehicle}</strong><small>{row.driver}</small><small className="timeline-row-period">{workPeriodStartedCopy[lang]} {row.dayOffset ? formatTimelineDateTime(row.periodStart, lang) : formatTimelineTime(row.periodStart, lang)}</small>{showRowDate || row.dayOffset ? <small className="timeline-row-date">{formatReportDate(row.actualDate, lang)}{row.dayOffset ? ` · +${row.dayOffset} ${lang === 'th' ? 'วัน' : row.dayOffset === 1 ? 'day' : 'days'}` : ''}</small> : null}{onPrintWorkPeriod ? <button className="timeline-print-button" type="button" onClick={() => onPrintWorkPeriod(row.reports[0])} disabled={!row.reports[0]?.vehicleNumber || !row.reports[0]?.workPeriodId}>{printWorkReportCopy[lang]}</button> : null}</div><div className="timeline-row-chart"><div className="timeline-row-scale" aria-hidden="true"><span>{formatTimelineTime(row.scaleStart, lang)}</span><span>{formatTimelineTime(row.scaleEnd, lang)}</span></div><div className="timeline-track timeline-speed-track" style={{ '--timeline-lane-count': row.laneCount }}>{row.segments.map(segment => <button
            ...
            aria-label={`${segment.accessibleLabel}${onOpenReport ? ` ${t.openGps}.` : ''}`}
            aria-haspopup={onOpenReport ? 'dialog' : undefined}
            ...
            onClick={event => {
              if (onOpenReport) { setTooltip(null); onOpenReport(segment.report); return; }
              if (tooltip?.segment.id === segment.id && tooltip.pinned) setTooltip(null); else openTooltip(event, segment, true);
            }}
          ><span className="sr-only">{segment.title}</span>{segment.finishWork ? <span className={`timeline-finish-marker ${segment.detail.status === t.cancelled ? 'cancelled' : ''}`} aria-hidden="true" /> : null}</button>)}<SpeedTimelineOverlay reports={row.reports} samplesByReportId={speedSeries.samplesByReportId} telemetryByReportId={speedSeries.telemetryByReportId} loading={speedSeries.loading} telemetryLoading={speedSeries.telemetryLoading} lang={lang} originTime={new Date(row.scaleStart).toISOString()} startMinute={0} endMinute={row.scaleDuration} /><TimelineAlertMarkers alerts={rowAlerts} lang={lang} startMinute={0} endMinute={row.scaleDuration} /></div><TimelineAlertChips alerts={rowAlerts} lang={lang} /></div></div>; })}
        </div>
        {tooltip ? <aside className="timeline-detail-tooltip" id={tooltip.segment.tooltipId} role="tooltip" style={{ left: tooltip.left, top: tooltip.top, width: tooltip.width, transform: tooltip.above ? 'translateY(-100%)' : undefined }}>
          ...
          {onOpenReport ? <p className="timeline-tooltip-action">{t.openGps}</p> : null}
        </aside> : null}
