...
export function buildReportQuery(searchParams) {
  ...
  const dateExpression = filters.dateBasis === 'job' ? 'report.start_time' : 'report.work_period_start_time';
  if (filters.startDate) clauses.push(`${dateExpression} >= (${parameter(filters.startDate)}::date::timestamp AT TIME ZONE 'Asia/Bangkok')`);
  if (filters.endDate) clauses.push(`${dateExpression} < ((${parameter(filters.endDate)}::date + 1)::timestamp AT TIME ZONE 'Asia/Bangkok')`);
