// forked from design-sync lib/css.mjs — rewriteBundleFontFaces dead-check regex
// drops rewritten quoted urls (optional-quote backtracking); check captured url instead.
...
export function rewriteBundleFontFaces({ out, bundleCss }) {
  ...
  const next = css.replace(/@font-face\s*\{[^}]*\}/gi, (block) => {
    ...
    // FORK FIX: the original optional-quote check backtracked (['"]? matched
    // empty, lookahead saw the quote) and dropped every REWRITTEN quoted url.
    // Check the captured url itself instead.
    const stillDead = [...b.matchAll(/url\(\s*['"]?([^'")]+)['"]?\s*\)/gi)].some((mm) => !/^(?:https?:|data:|\.\/fonts\/)/.test(mm[1]));
    if (stillDead) { dropped++; return '/* @ds-font-face-dropped: unresolvable src */'; }
    return b;
  });
  if (rewrote || dropped) {
