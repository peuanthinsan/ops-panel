...
The primary Next.js dashboard opens at `http://localhost:5173` and the local API runs at `http://localhost:4000`. The command starts the API when needed or reuses a compatible Songdee API already running on port 4000, so dashboard login works without a second terminal. Use the local default admin password `songdee-admin`.
...
The current signed preview release is Android version code 9. Its SHA-256 digest is `b91ac10e5728e99d1da081061b7926da2ca6104d4b215e30714d5f55f62cbda2`, and it is available from EAS build [`7fc81ab1-88ba-4ac5-a6b3-ffa501477274`](https://expo.dev/accounts/peuanthinsan/projects/songdee-ops-panel/builds/7fc81ab1-88ba-4ac5-a6b3-ffa501477274). APK binaries stay outside Git and are ignored under `releases/`.
...
Both profiles use EAS-managed Android version codes with automatic incrementing. The committed `extra.eas.projectId` belongs only to Ops Panel and is guarded by the release-identity test.

The local `server.js` development password defaults to `songdee-admin`. Production has no default password and requires the initial `SONGDEE_ADMIN_PASSWORD` environment variable.
...
- If the separate local binding record is missing, a job that already started—or has a durable final Done/Cancel payload—can reconstruct its exact vehicle/device pair from active-job recovery state and finish safely during an API outage. A selection still waiting for movement cannot do this and must revalidate against the server first.
- The nine requested titles and descriptions match the supplied control-panel wireframe. Selecting options 1–8 opens a start confirmation; option 9 becomes available only after movement starts and then confirms and saves the completed job.
- While movement is pending, all nine mode tiles are disabled and the header cancel control remains available. Once movement starts, only option 9 and Cancel job can end the active job.
...
- Start, Done, and Cancel confirmations expose a modal accessibility boundary, support the platform escape/back action, announce their heading, move TalkBack focus into that heading when opened, and return focus to the mode or Cancel control that invoked them when closed.
...
- While a job is active, the header provides a cancel control. Cancellation is confirmed and recorded on the dashboard with status `Cancelled`; cancelled jobs do not require a GPS lookup.
...
- Before the first Done or Cancel network attempt, the tablet also stores the exact final report payload in active-job recovery state. If SQLite is unavailable or the server response is lost, every in-session or post-restart retry reuses the same report ID, end time, duration, and cancellation status instead of creating an idempotency conflict.
