...
The dashboard timeline, job list, and printed reports also request `GET /api/admin/reports/:reportId/telemetry`. This admin-only endpoint calls Data-FM `GetFuelStatus` using the saved report's vehicle and start/end times, returning paired `speedKph` and `totalFuel` readings. It uses the same backend credentials, works without saved tablet GPS samples, and needs no database migration. The job list shows the latest available Total fuel reading within each job, with its timestamp, in both desktop rows and mobile cards. Concurrent list and timeline reads share a two-request queue and bounded short-lived caches. Fuel history is read on demand, splitting requests longer than one day into daily windows, up to seven days per job. Unavailable fuel history leaves the saved GPS speed trace visible. Both series share time and use separately labelled scales; missing readings and gaps over five minutes break the lines. `totalfuel` is displayed as Total fuel without assuming litres, and Data-FM's day-first fuel timestamps are parsed in the configured source timezone.
