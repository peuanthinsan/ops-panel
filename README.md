...
The dashboard timeline and printed reports also request `GET /api/admin/reports/:reportId/telemetry`. This admin-only endpoint calls Data-FM `GetFuelStatus` using the saved report's vehicle and start/end times, returning paired `speedKph` and `totalFuel` readings. It uses the same backend credentials, works without saved tablet GPS samples, and needs no database migration. Fuel history is read on demand with bounded short-lived caches; requests longer than one day are split into daily windows, up to seven days per job. Unavailable fuel history leaves the saved GPS speed trace visible. Both series share time and use separately labelled scales; missing readings and gaps over five minutes break the lines. `totalfuel` is displayed as Total fuel without assuming litres, and Data-FM's day-first fuel timestamps are parsed in the configured source timezone.
