...
test('work-period report is anchor-selected, speed-enabled, and uses the official logo pin', async () => {
  ...
  assert.match(speedTimeline, /reportTelemetryPoints\(reports, telemetryByReportId, samplesByReportId, originTime\)/);
  assert.match(speedTimeline, /className="speed-line"/);
  assert.match(speedTimeline, /className="fuel-line"/);
  assert.match(speedTimeline, /telemetryMarkerPoints\(next\)/);
  assert.match(speedTimeline, /role="slider" tabIndex=\{0\}/);
  assert.match(speedTimeline, /ArrowRight: 1/);
  assert.match(speedTimeline, /onPointerDown=/);
  assert.match(speedTimeline, /\{copy\.speed\}/);
  assert.match(speedTimeline, /\{copy\.fuel\}/);
