...
function bangkokDayBoundary(dayKey, end = false) {
  const match = String(dayKey || '').match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!match) return NaN;
  return Date.UTC(Number(match[1]), Number(match[2]) - 1, Number(match[3])) - (7 * 60 * 60 * 1000) + (end ? 24 * 60 * 60 * 1000 : 0);
}
...
function matchesQuery(report, query) {
  if (query.dayKey && !(Date.parse(report.startTime) < bangkokDayBoundary(query.dayKey, true) && Date.parse(report.endTime) >= bangkokDayBoundary(query.dayKey))) return false;
