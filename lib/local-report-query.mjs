const sortKeys = new Set([
  ...
  'mode', 'duration', 'status', 'gpsState', 'gpsCoverage', 'startClock', 'topSpeed',
]);
...
function vehicleKey(value) { return String(value || '').trim().toLocaleLowerCase(); }
...
function sortValue(report, key) {
  ...
  if (key === 'gpsCoverage') return Number(report.deviceGpsSamples) || 0;
...
export function localReportFacets(reports) {
  ...
  return {
    vehicles: [...new Map(reports.map(report => [vehicleKey(report.vehicleNumber), report.vehicleNumber]).filter(([key]) => key)).values()]
      .sort((left, right) => String(left).localeCompare(String(right), undefined, { numeric: true })),
...
export function queryLocalReports(reports, bindings, searchParams) {
  ...
  const filters = {
    ...
    startDate: String(searchParams.get('startDate') || '').trim(),
    endDate: String(searchParams.get('endDate') || '').trim(),
    dateBasis: searchParams.get('dateBasis') === 'job' ? 'job' : '',
    workPeriodId: String(searchParams.get('workPeriodId') || '').trim(),
    ...Object.fromEntries(['vehicle', 'device', 'driver', 'mode', 'status', 'gps']
      .map(key => [key, [...new Set(searchParams.getAll(key).map(value => String(value || '').trim()).filter(Boolean))]])),
  };
  const search = filters.search.toLocaleLowerCase();
  const periodReports = annotateWorkPeriods(reports);
  const filtered = periodReports.filter(report => {
    const date = filters.dateBasis === 'job'
      ? bangkokDateKey(report.startTime || report.start_time)
      : report.workPeriodDate || bangkokDateKey(report.startTime);
    return (!filters.startDate || date >= filters.startDate)
      && (!filters.endDate || date <= filters.endDate)
      && (!filters.workPeriodId || report.workPeriodId === filters.workPeriodId)
      && (!filters.vehicle.length || filters.vehicle.some(value => vehicleKey(report.vehicleNumber) === vehicleKey(value)))
      && (!filters.device.length || filters.device.includes(report.deviceId))
      && (!filters.driver.length || filters.driver.includes(report.driverName))
      && (!filters.mode.length || filters.mode.includes(report.mode))
      && (!filters.status.length || filters.status.includes(report.status))
      && (!filters.gps.length || filters.gps.includes(gpsState(report)))
  ...
  return {
    ...
    summary: {
      total,
      activeVehicles: new Set(filtered.filter(report => report.status !== 'Cancelled').map(report => vehicleKey(report.vehicleNumber))).size,
      ...
      // A vehicle can have multiple tablets; count physical vehicles once.
      fleetSize: new Set(bindings.map(binding => vehicleKey(binding.vehicleNumber)).filter(Boolean)).size,
    },
    pageInfo: {
